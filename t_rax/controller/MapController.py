...
from ..widget.MapWidgets import Map2DWidget
from ..widget.MapWidgets import ManualMapPositionsDialog
from ..widget.MapWidgets import OpenBGImageDialog
from ..widget.Widgets import save_file_dialog, open_files_dialog
...
class MapController(object):
    def __init__(self, working_dir, widget, model):
        """
        :param widget: Reference to IntegrationWidget
        :param model: Reference to BaseModel object
    ...
    def btn_roi_add_clicked(self, params):
        ...
        self.widget.graph_widget.add_item(self.map_widget.map_roi[roi_count]['Obj'])
    ...
    def btn_roi_add_phase_clicked(self):
        ...
        for line in phase_lines[0:5]:
            tc = {'x_center': self.map_model.convert_units(line[0], 'd_A', self.map_model.units,
    ...
    def make_roi_changed(self, curr_map_roi):
        def roi_changed():
            if self.map_widget.auto_update_map_cb.isChecked():
                return
            x_start, x_end = self.map_widget.map_roi[curr_map_roi]['Obj'].getRegion()
            row = self.map_widget.roi_list.row(self.map_widget.map_roi[curr_map_roi]['List_Obj'])
            new_roi_name = self.generate_roi_name(x_start, x_end, row)
    ...
    def myMouseClickEvent(self, ev):
        if ev.button() == QtCore.Qt.RightButton or \
            ...
            if self.map_model.map_uses_patterns:
                ...
                self.model.load_file(str(file_name))
    ...
    def map_window_raised(self):
        self.widget.batch_mode_mapping_rb.setChecked(True)
